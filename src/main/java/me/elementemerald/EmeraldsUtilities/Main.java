...
public class Main extends JavaPlugin implements Listener, TabCompleter {
	...
	public void onEnable()
	{
		...
		Bukkit.getScheduler().scheduleAsyncRepeatingTask(this, new ServerTPSResponse(), 0L, 20L);
			...
			if (s.hasPermission("EUtilities.view"))
			{
				try
				{
					...
					if (op != null)
					{
						...
						String[] msgs = {String.format("User ID: %s", uid.toString()), String.format("User IP currently connected: %s", Bukkit.getPlayer(args[0]).getAddress().toString()), String.format("User banned?: %s", banned ? "Yes" : "No"), String.format("User online?: %s", online ? "Yes" : "No"), String.format("User last played: %s", lastplayed)};
						s.sendMessage(msgs);
					}
					else {
